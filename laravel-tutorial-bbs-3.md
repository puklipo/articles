...
## 付録：本番環境への公開
最初はLaravel Forgeを使ってAWS EC2やVPSでの公開が推奨。Forgeは「Laravelを動かすための最適なサーバー構築」まで行ってくれるのでまずはここから。

最終的にはLaravel Vaporを使ってAWS Lambdaで動かすのが最高のLaravel本番環境だけどVapor特有の癖もあるのでLaravelに十分慣れてから。

レンタルサーバーは絶対に使ってはいけない。説明不要なほど絶対にありえない。
...
「HTTPリクエストを受け取ってレスポンスを返す」Http側とはまた違ったLaravelの側面が見える。
